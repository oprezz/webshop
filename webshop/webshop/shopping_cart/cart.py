    ...
    if not cint(cart_settings.allow_items_not_in_stock):
        for item in sales_order.get("items"):
            item.warehouse = frappe.db.get_value(
                "Website Item", {
                    "item_code": item.item_code}, "website_warehouse"
            )
...
def update_cart_address(address_type, address_name):
    ...
    if address_type.lower() == "billing":
        ...
        address_doc = next(
            (doc for doc in get_billing_addresses()
        ...
        address_doc = next(
            (doc for doc in get_shipping_addresses()
             if doc["name"] == address_name),
            None,
        )
    apply_cart_settings(quotation=quotation)
...
def decorate_quotation_doc(doc):
    for d in doc.get("items", []):
        ...
        fields = ["web_item_name", "thumbnail",
                  "website_image", "description", "route"]
...
def _set_price_list(cart_settings, quotation=None):
    ...
    party_name = quotation.get(
        "party_name") if quotation else get_party().get("name")
...
def get_party(user=None):
    ...
    if party:
        ...
        contact.update(
            {"first_name": fullname, "email_ids": [
                {"email_id": user, "is_primary": 1}]}
        )
...
def get_applicable_shipping_rules(party=None, quotation=None):
    ...
    if shipping_rules:
        rule_label_map = frappe.db.get_values(
            "Shipping Rule", shipping_rules, "label")
...
def get_customer_settings(doc, party=None):
    ...
    exceeds_minimum_order_value = True
