...
from webshop.webshop.utils.product import (
    get_non_stock_item_status, get_web_item_qty_in_stock)
...
def get_product_info_for_website(item_code, skip_quotation_creation=False):
    ...
    if product_info["price"]:
        if frappe.session.user != "Guest":
            item = (
                cart_quotation.get({"item_code": item_code}
    ...
    product_weekly_availability = website_item.get("custom_available_on")

    if product_weekly_availability:
        # append days to a list
        product_info["weekly_availability"] = [
            day.get("day") for day in product_weekly_availability
        ]
...
def set_product_info_for_website(item):
    ...
    if product_info:
        ...
        if product_info.get("price"):
            item["price_stock_uom"] = product_info.get(
